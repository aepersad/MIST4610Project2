...
## Data Manipulations: 
There were 26 data cleaning manipulations done to the data set. There were 6 columns that were renamed to more descriptive names. 4 columns had values replaced with clearer or shorter versions. 3 of the columns were split into separate columns, and another 3 had values that were blank replaced with a label. One column was created based on values in an existing column and 9 columns were removed due to redundancy or carrying no useful information. For a detailed breakdown of each manipulation, see the [Appendix.](#appendix)

## Tableau Packaged Workbook: 
The packaged workbooks containing the visualizations shown above are attached to this repository.
